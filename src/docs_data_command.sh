...
# docs_data_command.sh - Display data API documentation
...
print_info "Fetching data API documentation from: $current_server"

# Fetch documentation from API endpoint
if docs_content=$(curl -s --max-time 30 --fail "$base_url/docs/data" 2>/dev/null); then
